...
//更新，并返回修改记录条数
func (h *GroupHandler) UpdateForRowAffected(update *UpdateDef) (int64, error) {
	if h.DbMaster.Status {
		updatestr := update.RawUpdate
		if updatestr == "" {
			colstr := strings.Join(update.UpdateCols, ",")
		...
		return updateDbForRowAffected(h.DbMaster.Db, updatestr, update.Vals...)
	}
	return 0, errors.Errorf("%s's master is offline", h.Name)
}
...
func (h *GroupHandler) InsertReturnPrimaryKeyValue(insert *InsertDef) (int64, error) {
	if h.DbMaster.Status {
		insertstr := insert.RawInsert
		vals := insert.InsertVals
		if insertstr == "" {
		...
		return insertDbReturnLastInsertId(h.DbMaster.Db, insertstr, vals...)
	}
...
//插入并返回主键
func insertDbReturnLastInsertId(dbMaster *sql.DB, query string, args ...interface{}) (int64, error) {
	result, err := dbMaster.Exec(query, args...)
	if err != nil {
		return 0, errors.Wrap(err, "insertDB")
	}
	return result.LastInsertId()
}

//插入并返回影响条数
func insertDbReturnAffected(dbMaster *sql.DB, query string, args ...interface{}) (int64, error) {
	result, err := dbMaster.Exec(query, args...)
	if err != nil {
		return 0, errors.Wrap(err, "insertDB")
	}
...
func updateDbForRowAffected(dbMaster *sql.DB, query string, args ...interface{}) (int64, error) {
	r, err := dbMaster.Exec(query, args...)
	if err != nil {
		return 0, errors.Wrap(err, "updateDb")
	}
