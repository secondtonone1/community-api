...
func (h *GroupHandler) Count(query *QueryCountDef) (int64, error) {

	if h.DbSlave.Status {

		if query.Table != "" {
			countField := query.CountField
			if countField == "" {
				countField = " count(1) "
			}
			var querykeysstr string
			...
			querystr := "select " + countField + " from " + query.Table + " where " + querykeysstr
			return queryCountDb(h.DbSlave.Db, querystr, query.Vals...)
		}
		return 0, errors.New("Count table is null")
	}

	return 0, errors.New("GetOne db is offline")
}

//count表中数据
func queryCountDb(dbconn *sql.DB, query string, args ...interface{}) (int64, error) {

	rows, err := dbconn.Query(query, args...)
	if err != nil {
		logging.Logger.Error("db", "queryCountDb fail", query, args, err.Error())
		return 0, err
	}
	defer rows.Close()
	for rows.Next() {
		var countNum int64
		err = rows.Scan(&countNum)
		return countNum, err
	}
	return 0, nil
}
